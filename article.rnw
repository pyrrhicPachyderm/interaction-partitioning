...
In our grouping, we avoided the common assumption that species which are similar in their responses to competition would automatically be similar in their competition effects.
As such, we had two groupings; one grouping of the competitive responses, and one of the competitive effects.
...
Groupings are known formally as ``partitions'' in mathematics.
A grouping divides the species into groups such that each species belongs to one and only one group.
