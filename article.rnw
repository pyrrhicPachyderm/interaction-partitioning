...
% DBS: We should have a conversation regarding authorship. To me, it makes perfect sense thus far that you are the sole author. Depending on how revisions go and what anyone else or I do or do not contribute, it may be appropriate for this to change. In many such instances, I often think that the lead author (i.e., you) will have the best feeling for this relative to the criteria set out in the Vancouver guidelines for authorship.
...
Even the simplest models with no environmental dependence require one parameter to characterize the interaction of each pair of species \parencite{novak2016characterizing, XXX}.
...
To bridge this gap between XXX and YYY, we need an ecologically-defensible way to reduce the number of parameters that predictive models require.
...
In the statistical literature, regularisation is the process of using external information to prevent overfitting or to solve an otherwise overspecified problem \parencite{XXX}. One simplifying approach is to assume that most pairs of species do not interact or interact weakly, and hence setting many of the parameters to zero \parencite{XXX}. XXX why this could be a problem XXX
...
The argument for grouping based on phylogeny is based on the principle (originally hypothesised by Darwin himself \cite{XXX}) that closely related species will be similar in (what we now know as) ecological niche, and thereby will interact with other species in a similar fashion \parencite{godoy2014phylogenetic}.
% DBS: the preceding sentence could be simplified a fair bit
...
In all, we are left with the question of how best to group species in community models, and whether we can do so in a meaningful fashion with less data than required for a fully pairwise parameterisation.

I propose that species might be best partitioned into groups without regard to any \emph{a priori} definition but instead based purely upon which grouping gives the most parsimonious model.
...
In this study, I investigate the usefulness and correctness of such an approach using experimental \emph{Drosophila} fecundity data provided by \textcite{terry2021natural}. I fit the Lotka--Volterra model, with grouped species, to find the grouping that gives the most parsimonious model.
...
% DBS: I feel like the last sentences of this paragraph, from "In this study,..." onward should be rephrased to be more user friendly and without introducing new concepts (e.g., rows vs columns). If that info is important for the introduction, it ought to appear before the "This is what we do here" paragraph.
...
This experimental design corresponds to a response--surface experimental design commonly used to parameterise a species interaction model \textcite{hart2018quantify, XXX}.
...
% DBS: CRPB to fix this response variable when he fixes the model formula to reflect number of offspring.
...
% DBS: We should probably remove all of the following except the bit about a single founding pair since it's the only thing with direct bearing on how this model gets fit. For reference, it's not hard to add an Allee effect to a LV model if we wanted to add another set of parameters, though I understand why we probably don't!
The experiment also included a number of other vials that were excluded from the analysis for various reasons.
...
As the response variable was fecundity, I used a discrete-time version of the Lotka--Volterra competition model given by:
\[
	N_{i,t+1} = N_{i,t} r_i \left(1 - \sum_j \alpha_{ij} N_{j,t}\right)
,\]
% DBS: CRPB to update the equation above after consideration of experimental response variable.
where $N_{i,t}$ is the population of species $i$ at time $t$, $r_i$ is the intrinsic growth rate of species $i$, and $\alpha_{ij}$ is the \emph{per capita} competitive effect of species $j$ on species $i$.
...
% DBS: Similar to my comment above, I'm not sure the rows vs column groupings is sufficiently introduced to this point. So I think we need more than a single sentence explaining this idea. Since it appears below, we can reiterate the meaning in terms of equivalence of competitive responses and equivalence of competitive effects.
...
Each model fit was performed with one grouping of the interaction matrix's rows and a second, independent grouping of the matrix's columns.
...
With $1$ group, every species is placed in the same group, such that all are equal in their competitive effects or responses, reminiscent of the neutral model.
Intermediate numbers of groups give more freedom in the groupings, such that there are many possible groupings for any intermediate number of groups. % DBS: Is this sentence essential?
...
Species in the same column grouping were constrained to have the same competitive effect upon all other species, while species in the same row grouping were constrained to have the same competitive response to all other species.
Formally, if species $a$ and $b$ were in the same column grouping, $\alpha_{ia}\equiv\alpha_{ib} \, \forall i$, and if $a$ and $b$ were in the same row grouping, $\alpha_{aj}\equiv\alpha_{bj} \, \forall j$.
...
% DBS: I think my comments at the top of the previous section really boil down to simply reorganizing the pieces in the different paragraphs so that they more clearly build on each other linearly.
...
I assumed a normal error distribution of the response variable. and 
to prevent heteroskedasticity and improve normality, I transformed total fecundity to \emph{per capita} fecundity, which corresponds to the following model:
\[
	\frac{N_{i,t+1}}{N_{i,t}} = r_i \left(1 - \sum_j \alpha_{ij} N_{j,t}\right)
.\]
% DBS: CRPB to alter equation above consist with changes to previous equations.


\subsubsection{Assessing parsimony}

To assess the parsimony of the different species groupings, I exhaustively fit all \num{\Sexpr{numbers::bell(6)^2}} possible models.
As a measure of parsimony, I selected AIC over BIC on the basis that we want to find the best model for making ecological predictions, rather than seeking the ``true'' model \parencite{aho2014model}.
...
As such, I also converted the set of AIC values to Akaike weights \parencite{wagenmakers2004aic}.
...
% DBS: I find the last sentence above to be worded a bit cryptically.

%TODO: Explain the coclassification matrices and the tests.

\subsection{Phylogeny}
% DBS: For now, I would cut out the bit about phylogeny since it's relatively weak without a properly calibrated tree. If you want to do a post hoc test, I would go with one that looks at body size and co-classification matrices. On the other hand, my gut tells me that the paper stands on its own merits without a post hoc test, and this is an easy thing to guide reviewers toward commenting on/requesting that you also know how to address. So it might be a useful strategic omission.
...
\Textcite{terry2021natural} gave the subgroups of the genus to which each study species belongs.
\Textcite{lewis2005phylogeny} gave the relationship between three of these subgroups (melanogaster, ananassae, and montium), all of the subgenus Sophophora, while \textcite{izumitani2016phylogeography} described the fourth subgroup (nasuta) as belonging to the distinct Drosophila subgenus.
...
% DBS: I think the last sentence above needs a bit of elaboration here OR the usefulness of this "test" should be explained in the Methods in the section that discusses how we examined aspects of parsimony.
...
As such, I assessed the appropriate groupings by taking the total probability that whichever was the best model grouped a given pair of species together (i.e.\ coclassified the pair of species).
These results are shown in \cref{fig:coclassification}.
% DBS: Much of the paragraph above is probably more appropriate at Methods text whereas in the Results we should discuss the patterns that emerge from the co-classification matrices.
...
% DBS: As I have noted before, I'd like to know what the null expectation is for this before claiming confidence of being co-classified or not being co-classified. Firstly, I think that the random expectation is much lower than one might otherwise suspect. (For three species, it is already 2/5; for four species, it is 5/15.) Second, the conventional "ecological" null is actually that the probability is 0, in which case it doesn't take much co-classification probability to see the glass as half full and argue that grouping is a much more parsimonious approach than the contrary.
...
% DBS: In the network modularity literature, there is a nice measure of mutual information between two partitions. I wonder if we could exploit that here to give a quantitative measure of similarity between the two. Or we could just convert the two matrices to distance matrices and run a Procrustes to measure their similarity?
...
The phylogeny divided the six species into three pairs of most closely related species (\cref{fig:phylogeny}).
...
% DBS: I would start this paragraph with a restatement of the overall premise.
...
Alas, the brute force method I employed in this study has a very poor algorithmic complexity, preventing the application of the method to studies with more than seven or eight species.
