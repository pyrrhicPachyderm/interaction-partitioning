...
Alas, the brute force method we employed in this study has a very poor algorithmic complexity, preventing the application of the method to studies with more than seven or eight species (there are ${\left(B_7\right)}^2 = \num{\Sexpr{numbers::bell(7)^2}}$ possible models with $7$ species and ${\left(B_8\right)}^2 = \num{\Sexpr{numbers::bell(8)^2}}$ models with $8$ species).
