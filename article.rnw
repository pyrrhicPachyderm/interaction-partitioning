...
%Title Page
% DBS: What about "Simplifying Lotka--Volterra interaction matrices by species grouping", with or without the bit after the colon (and where I lean slightly in favor of the "with" option)
...
% DBS: add some sort of concluding sentence a la where to from here
...
However, weak interactions have been shown to be very important to the stability of ecological communities \parencite{XXX, wootton2016many}, and setting these to zero may lose important information.
...
In ecological contexts, such groups have been defined \emph{a priori} by phylogeny at some taxonomic level or by functional similarity \parencite{uriarte2004spatially,ovaskainen2017species,weiss2021disentangling}.
% please add a reference to http://dx.doi.org/10.1086/713082 to the above
The argument for grouping based on phylogeny relies on the principle that closely related species will fill similar ecological niches, and hence will interact with other species in a similar fashion \parencite{XXX,godoy2014phylogenetic}.
...
On the other hand, functional groupings have also proven to be poor predictors of species interactions \parencite{XXX, bret2008plant}.
...
To the best of our knowledge, studies that use grouping to simplify models of species interactions have also made one additional, untested assumption: that the same grouping is equally valid for how species affect others as for how species respond to others \parencite{XXX}.
...
% DBS: Maybe it's worth adding a concrete example here for the more empirical reader/reviewer.
...
In this study, we investigate the usefulness and correctness of such an approach using experimental data from competition between different \emph{Drosophila} species provided by \textcite{terry2021natural}.
We fit a Lotka--Volterra model to these data to find the groupings that give the most parsimonious model.
...
% DBS: If we leave in the traits, then we should mention them at least briefly.
...
% DBS: I don't think we need the above because we say the same thing in line 172 below.
...
%---this was the size of the next generation.
% DBS: Do we need the bit I've commented out above? Plus, it isn't the size of the next generation it's the number of new adults...
...
% DBS: Let's add something that very briefly outlines how many parameters this model has. Then, below, we can provide formulae for the number of parameters given the number of row and column groups. Even though people can work this out themselves, it is probably useful to make it explicit so that we don't have to depend on them actually mustering up the energy to do so.

% DBS: We might want to be prepared for reviewers asking us to change from a linear LV model.

\subsubsection{Species grouping}

% DBS: I still feel like the bits and pieces of the section below are not in a well-flowing order.
...
The Lotka--Volterra competition model in Eq.~(\ref{LVmodel}) is not linear in the parameters.
...
As such, each cell represents the probability that the most parsimonious model is one that coclassifies those two species. For reference, note that the diagonal of a coclassification matrix is tautologically equal to $1$.
...
\begin{figure}
	...
	\caption{%
		...
		This represents the single model with the best {\ic} (\probnum{\Sexpr{brute_data$statistics[best_model_index,ic]}}). %TODO: Debug this.
		\Cref{fig:best-alpha-matrix} shows the competition coefficients, where a competition coefficient in a block represents the same coefficient being used for multiple interactions, per the groupings.
		\Cref{fig:best-coclassification-cols} and \cref{fig:best-coclassification-rows} show the coclassification matrices derived from the groupings of the columns and rows respectively.
		...
		The ordering is the same in \cref{fig:best-coclassification-cols} and \cref{fig:best-coclassification-rows} as shown in \cref{fig:best-alpha-matrix}.
	}
...
% DBS: In general, I am more in favor of just putting something like a pointer to a figure "(Fig.~1)" at the end of the sentence below than an explicit statement of what appears in a figure. Some journals also frown on devoting words to explanations of what appears in figures since that is technically what figure captions are for.
Overall, no pair of species can confidently be said to be coclassified together in the row or column groupings; the highest probability of classification was \SI[round-mode=figures,round-precision=3]{\Sexpr{max(weighted_col_coclassification_matrix - diag(num_species), weighted_row_coclassification_matrix - diag(num_species))*100}}{\percent}.
...
% DBS: Within the caption, the reference to a subfigure should just be (a) and (b), not Figure 2a and fig. 2b.
...
This result stands in contrast to the popular wisdom in ecology that species should form some sort of clear groups, be they the functional groups so often used \parencite{XXX, blondel2003guilds}, or taxonomic groups per a relationship between phylogeny and ecological niche \parencite{XXX, godoy2014phylogenetic}.
...
% DBS: We're probably doing a disservice to the "groups in ecological communities" literature by only citing the Allesina paper above.

One possible explanation is that species simply do not group to the same extent in a competitive context.
...
Alas, the brute force method we employed in this study has a very poor algorithmic complexity, preventing the application of the method to studies with more than seven or eight species. %DBS: Add in the number of models for both of these to emphasize the point.
...
% DBS: I think we should swap the order of the following two paragraphs.

The intransitivity of coclassification in some instances even suggests that there may not be a single ``best'' grouping of the species, but rather two or more quite distinct and mutually exclusive groupings of similar validity. A single trait like body size might therefore not be able to capture the multivariate nature of species' ecological niches \parencite{XXX}.
This could suggest a Bayesian approach is worth exploring in the future, in which the outcome is not a definitive grouping but a distribution of valid groupings.
...
% DBS: Never shoot yourself in the foot in your concluding paragraph! The reviewers will already be primed to do this for you, so I always think it's better to take a glass half full perspective.
% DBS: Also, there is no hard and fast rule to say we need a "sum it all up" concluding paragraph. Each of the statements below is pretty much already stated above but with more depth. I think we could actually do away with it and lose very little.
