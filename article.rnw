...
The data provided the change population size from one generation to the next, so I used a discrete-time version of the Lotka--Volterra competition model given by:
\[
...
I assumed a normal error distribution of the response variable and to prevent heteroskedasticity and improve normality, I transformed total population growth to \emph{per capita} population growth, which corresponds to the following model:
\[
