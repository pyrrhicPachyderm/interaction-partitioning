...
One simplifying approach is to assume that most pairs of species do not interact or interact weakly, and hence setting many of the parameters to zero \parencite{mutshinda2009drives, weiss2021disentangling}.
...
In ecological contexts, such groups have been defined \emph{a priori} by phylogeny at some taxonomic level or by functional (i.e., trait) similarity \parencite{uriarte2004spatially,ovaskainen2017species,weiss2021disentangling,martyn2021identifying}.
...
% DBS: As per my comment immediately above, we need to add a reference to the partial residual plots in the supps.
...
\begin{figure}
	...
	<<>>=
		...
		abline(v = brute_data$fully_grouped_statistics[[ic]], lwd=2, lty="dotted", col="red")
	@
	\end{subfigure}
	...
	\caption{%
		...
		The dotted red line shows the {\ic} value (\icnum{\Sexpr{brute_data$fully_grouped_statistics[[ic]]}}) of the fully grouped model in which every pair of species shares a single interaction coefficient.
	}
	\label{fig:ic}
\end{figure}
% DBS: Reconsider green and red to be more colour-blind friendly. I've already changed it so that the lines are different types.
...
\begin{figure}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		@
		\caption{Coclassification matrix of rows}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		@
		\caption{Coclassification matrix of columns}
...
% DBS: I cut out a sentence above in the caption because we say this in the text elsewhere and it's a bit unusual to refer to a later figure in the caption of an earlier one.
...
% DBS: Adding a new note here. Shouldn't we pick one version that gets used in Fig 2 and in Fig 3 to refer to the two coclassification matrices?
...
In hindsight, this should not be overly surprising since it is known that different traits govern species competitive effects than govern their responses \parencite{goldberg1991competitive}. % DBS: I think it would be good to find another reference here. Maybe this one? https://onlinelibrary.wiley.com/doi/10.1111/j.1365-2486.2008.01557.x
...
% DBS: Can we replace "in some instances" below with something a bit more explicit? Do we mean "for some sets of species"?
...
A single trait like body size might therefore not be able to capture the multivariate nature of species' ecological niches \parencite{blonder2018hypervolume}.
% DBS: Should we consider replacing "ecological niches" above with "competitive niches"?
