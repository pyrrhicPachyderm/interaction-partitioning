...
I propose that groups might be selected without regard to any \emph{a priori} definition, based purely upon which grouping gives the most parsimonious model.
...
,\]
where $N_{i,t}$ is the population of species $i$ at time $t$, $r_i$ is the intrinsic growth rate of species $i$, and $\alpha_{ij}$ is the \emph{per capita} competitive effect of species $j$ upon species $i$.
...
With six species, and the row and column groupings chosen independently, the total number of possible models is ${\left(B_6\right)}^2 = \num{\Sexpr{numbers::bell(6)^2}}$.
...
With the rows grouped, however, this becomes impossible, for each species still has its own growth rate.
