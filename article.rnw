...
Most dynamic models of ecological communities require many parameters, making them expensive to fit to experimental or observational data.
...
Here, we assess the validity of such grouping by deriving the optimal groupings \emph{a posteriori} based on the fit of the Lotka--Volterra competition model to experimental data from six species of \emph{Drosophila} competing in laboratory conditions.
...
Further, those groupings that did prove beneficial could not have been easily predicted from available species attributes, calling into question the prevailing method of \emph{a priori} species grouping.
...
Our study suggests the need for a different, more data-informed, approach to grouping in species interaction models.

% DBS: Once we have a title, make sure that we remove all words from the keywords that appear there.
\keywords{competition; ecological equivalence; grouping; model fitting; parsimony; \emph{Drosophila}}
...
One simplifying approach is to assume that most pairs of species do not interact or interact weakly, and hence setting many of the parameters to zero \parencite{mutshinda2009drives, weiss2022disentangling}.
However, weak interactions have been shown to be very important to the stability of ecological communities \parencite{mccann1998weak,neutel2002stability,wootton2016many}. Setting any individual parameter to zero thus risks losing important information; moreover, we often do not know with confidence which parameters are likely to be zeros and which are not.
Another approach assumes that species fall into groups, such that all species in the same group interact with species in other groups in an identical fashion, thereby collapsing pairwise interaction parameters for all the species in that group into sets of common parameters \parencite{ovaskainen2017species}.

In ecological contexts, such groups have been defined \emph{a priori} by phylogeny at some taxonomic level or by functional (i.e., trait) similarity \parencite{uriarte2004spatially,ovaskainen2017species,martyn2021identifying}.
...
For resource competition in particular, competitive effects are determined by how the organism affects local resource availability while competitive responses are determined by how the organism is inhibited by depleted resources \parencite{violle2009competition}.
...
Such data-informed grouping approaches have previously proved successful in other contexts, such as finding modules in networks \parencite{guimera2005functional,newman2006modularity,girvan2002community}.
In this study, we investigate the usefulness and accuracy of such an \emph{a posteriori} approach using experimental data from competition between different \emph{Drosophila} species provided by \textcite{terry2021natural}.
We fit a Lotka--Volterra competition model to these data to find the groupings that give the most parsimonious model, and we then compare these groupings to similarity based upon a species trait (body size).
...
The data used in this study were drawn from the work of \textcite{terry2021natural}.
They consist of fecundity measurements---number of surviving offspring---for six species of \emph{Drosophila}, grown in single-species or two-species communities with varying densities of each species.
This experimental design corresponds to a response--surface experimental design commonly used to parameterise species interactions in population dynamics models \parencite{hart2018quantify,zhang2019common,cameron2019size}.
...
The number of adults of each species emerging from these eggs was counted.
...
The two-species densities were (3,3), (3,6), (3,9), (3,12), (6,3), (9,3), or (12,3) breeding pairs per vial (where the paired values give the densities of each of the two species), and each of these was replicated three times per pair of species.
...
\begin{equation}
	N_{i,t+1} - N_{i,t} = N_{i,t} \, r_i \left(1 - \sum_j^n \alpha_{ij} N_{j,t}\right) \,,
	\label{eq:lv-model}
\end{equation}
...
When fitting the data to the model, the competition coefficients were not constrained to be positive; this allows for the possibility of mutualistic effects between some species.
With $n$ species, this model has $n+n^2$ parameters: $n$ growth rates and $n^2$ competition coefficients.

% DBS: A tardy thought, if we don't constrain alphas I believe many people would say we should claim that we are fitting the generalised Lotka--Volterra model and *not* a Lotka--Volterra competition model.
...
At the other extreme, all species could be placed in the same group, such that all pairs of species interact equivalently; they are ecologically equivalent \emph{sensu} \textcite{mcpeek2019disentangling}.
Given $n$ species, there may be any integer number of groups from $1$ to $n$.
Groupings are known formally as partitions in mathematics, and the total number of ways to group $n$ species is given by the Bell number, denoted $B_n$ \parencite{bell1934exponential}.
...
However, competitive responses may be governed by a different set of traits than competitive effects, so this assumption may not always hold.
We therefore relaxed this assumption by allowing species which were similar in one regard to be dissimilar in the other, and vice versa.
As such, each grouped model was defined by two groupings of species: one grouping of the competitive responses and one of the competitive effects.

To better understand the meaning of species grouping of competition coefficients, it is useful to recognise that this component of the Lotka--Volterra competition model can be represented as an interaction matrix \parencite{novak2016characterizing}, where a row contains all the competitive effects on a single species and a column contains all the competitive effects of a single species.
If two species are assumed to have the same responses to competition, then two rows of the interaction matrix are constrained to be equal.
Similarly, if two species are assumed to have the same competitive effects, then two columns of the interaction matrix are constrained to be equal.
...
Second, the total Akaike weight of models that used the same grouping for the rows as for the columns; this is the probability that the models grouping species equivalently in their competitive effects and competitive responses were more parsimonious than all other models.
To evaluate the significance of the latter, we performed a Monte Carlo permutation test, comparing the total Akaike weight of those models that grouped rows and columns equivalently to the total Akaike weight of an equally sized random subset of the models.
...
By testing for correlation between the coclassification matrix for the the rows and that for the columns, we could perform a second test of the assumption that it is appropriate to use the same grouping for both.
...
We then performed Mantel tests for correlation between the trait distance matrix and each of the row-grouping and column-grouping coclassification dissimilarity matrices.

\section{Results}

Plots of predicted against observed values and partial residual plots for the fitted alpha values are shown in \cref{fig:model-fit-best,fig:model-fit-average,fig:partial-residual-best,fig:partial-residual-average}.
...
\begin{figure}
	...
	<<>>=
		ic_histogram(brute_data)
	@
	\end{subfigure}
	
	\caption{%
		Histogram of the {\ic} values of Lotka--Volterra model fits with all possible groupings of interaction coefficients.
		The dashed green line shows the {\ic} value (\icnum{\Sexpr{brute_data$fully_separated_statistics[[ic]]}}) of the ungrouped model in which all competition coefficients are fitted independently.
		The dotted purple line shows the {\ic} value (\icnum{\Sexpr{brute_data$fully_grouped_statistics[[ic]]}}) of the fully grouped model in which every pair of species shares a single interaction coefficient.
	}
	\label{fig:ic}
\end{figure}
...
This indicates that the most parsimonious model was not especially likely to be one that grouped the rows and columns equivalently.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		<<>>=
			cat(best_model_output_table$grouped_alpha_matrix)
		@
		\caption{Grouped alpha matrix}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		<<>>=
			cat(best_model_output_table$row_coclassification_table)
		@
		\caption{Coclassification of competitive effects\\(columns)}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		<<>>=
			cat(best_model_output_table$row_coclassification_table)
		@
		\caption{Coclassification of competitive responses\\(rows)}
	...
	\caption{%
		...
		Where results are for the single model with the best {\ic} (\probnum{\Sexpr{brute_data$statistics[best_model_index,ic]}}).
		\Subref{fig:best-alpha-matrix} We show the inferred matrix of competition coefficients, where a competition coefficient in a block represents the same coefficient being used for multiple interactions (per the identified groupings).
		\Subref{fig:best-coclassification-cols} and \subref{fig:best-coclassification-rows} We show the coclassification matrices derived from the groupings of the columns and rows, respectively.
		Species have been reordered so that species that were grouped together appear adjacent, and the ordering is the same in all of \subref{fig:best-alpha-matrix}, \subref{fig:best-coclassification-cols}, and \subref{fig:best-coclassification-rows}.
	}
...
The structure of the coclassification matrices was very different in the row groups than in the column groups.
That is, most pairs of species which had a high probability of being coclassified in one grouping had a very low probability of being coclassified in the other grouping.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\centering
		<<>>=
			cat(weighted_coclassification_matrices$col)
		@
	...
	\begin{subfigure}{\textwidth}
		\centering
		<<>>=
			cat(weighted_coclassification_matrices$row)
		@
	...
	\caption{%
		...
		\Subref{fig:coclassification-cols} We show the grouping of species' competitive effects upon other species (columns of the interaction matrix). \subref{fig:coclassification-rows} We show the grouping of species' responses to competition (rows of the interaction matrix).
	}
...
Indeed, the results show that grouping species was likely to give a more parsimonious model (probability \probnum{\Sexpr{sum(brute_data$statistics[[ic_weight]][brute_data$statistics[[ic]] < brute_data$fully_separated_statistics[[ic]]])}}) than was leaving them ungrouped.
...
It also contrasts with previous studies that have succeeded in finding clear groups in other ecological contexts, such as in food webs \parencite{krause2003compartments,allesina2009food} or plant--pollinator networks \parencite{olesen2007modularity}.

One possible explanation for our results is that species simply do not group to the same extent in a quantitative competitive context as in a binary interaction networks.
That said, the results also suggest that this is unlikely as grouping was useful for increasing parsimony.
...
A study system with species spanning multiple functional groups, or a broader taxonomy, might have shown less equivocal groupings.
Alternatively, the small number of species in the study might not have been sufficient for clear groups to be defined.
Alas, the brute force method we employed in this study has a very poor algorithmic complexity, preventing an application of this approach to studies with more than seven or eight species (there are ${\left(B_7\right)}^2 = \num{\Sexpr{numbers::bell(7)^2}}$ possible models with $7$ species and ${\left(B_8\right)}^2 = \num{\Sexpr{numbers::bell(8)^2}}$ models with $8$ species).
...
While the results could not definitely identify a single best grouping, it was clear that the model-supported groupings differed for the rows and columns.
...
In hindsight, this should not be overly surprising since it is known that different traits govern species competitive effects than govern their responses \parencite{goldberg1990components, goldberg1991competitive}; for example shading effect and shade tolerance in plants.
...
Our results recommend more explicit questioning, and even testing, of this assumption when building future species interaction models.
%TODO: Discuss the fact that species were coclassified more in the rows than in the columns?

The intransitivity of coclassification for some sets of species even suggests that there may not be a single ``best'' grouping of the species, but rather two or more quite distinct and mutually exclusive groupings of similar validity.
A single trait like body size might therefore not be able to capture the multivariate nature of species' competitive niches \parencite{blonder2018hypervolume}.
This could indicate that a Bayesian approach is worth exploring in the future, in which the outcome is not a definitive grouping but a distribution of valid groupings.
...
I would like to thank Hao Ran Lai for feedback on a draft of the manuscript.

DBS is also grateful for the support of the Marsden Fund Council, from New Zealand Government funding (grant 16-UOC-008).
