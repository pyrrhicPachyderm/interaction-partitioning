...
% DBS: Wait, the response is pairs? So can we have non-integer responses?
...
\begin{figure}
	...
	<<>>=
		...
		abline(v = brute_data$fully_separated_statistics[[ic]], lwd=2, lty="dotted", col="green")
	...
	\caption{%
		...
		The dotted green line shows the {\ic} value (\icnum{\Sexpr{brute_data$fully_separated_statistics[[ic]]}}) of the ungrouped model in which all competition coefficients are fitted independently.
		The dashed red line shows the {\ic} value (\icnum{\Sexpr{brute_data$fully_grouped_statistics[[ic]]}}) of the fully grouped model in which every pair of species shares a single interaction coefficient.
	}
	\label{fig:ic}
\end{figure}
