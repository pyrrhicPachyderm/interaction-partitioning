...
At the other extreme, all species could be placed in the same group, such that all pairs of species interact equivalently, reminiscent of the neutral model.
...
However, competitive responses may be governed by a different set of traits than compeititive effects, so this assumption does not necessarily hold.
