...
<<include=FALSE>>=
	...
	ic <- "aic" #The information criterion to use, e.g. "aic" or "aicc"
...
The AIC values ranged from \aicnum{\Sexpr{min(brute_data$statistics[[ic]])}} to \aicnum{\Sexpr{max(brute_data$statistics[[ic]])}}.
The ungrouped model gave an AIC value of \aicnum{\Sexpr{brute_data$fully_separated_statistics[[ic]]}}.
...
\begin{figure}
	...
	\caption{%
		...
		The green line shows the AIC value (\aicnum{\Sexpr{brute_data$fully_separated_statistics[[ic]]}}) of the ungrouped model, in which all competition coefficients are fitted independently.
		The red line shows the AIC value (\aicnum{\Sexpr{brute_data$fully_grouped_statistics[[ic]]}}) of the fully grouped model, where every pair of species shares a single interaction coefficient.
	}
	\label{fig:aic}
\end{figure}

The ``best'' model, as indicated by AIC, was not the best with any surety; its Akaike weight indicated a probability of only \probnum{\Sexpr{max(brute_data$statistics[[ic_weight]])}} that it was the best.
...
In all, no pair of species can confidently be said to be coclassified in the row or column groupings; the highest probability of classification was \SI[round-mode=figures,round-precision=3]{\Sexpr{max(weighted_col_coclassification_matrix - diag(brute_data$num_species), weighted_row_coclassification_matrix - diag(brute_data$num_species))*100}}{\percent}.
