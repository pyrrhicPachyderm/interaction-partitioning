...
<<include=FALSE>>=
	...
	brute_data <- BruteData$new("output/brute.data", species_names)
...
In the statistical literature, regularisation is the process of using external information to prevent overfitting or to solve an otherwise overspecified problem \parencite{XXX}.
One simplifying approach is to assume that most pairs of species do not interact or interact weakly, and hence setting many of the parameters to zero \parencite{XXX}. %TODO: XXX why this could be a problem XXX.
...
I assumed a normal error distribution of the response variable and to prevent heteroskedasticity and improve normality, I transformed total fecundity to \emph{per capita} fecundity, which corresponds to the following model:
\[
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		...
		<<echo=FALSE>>=
			weighted_coclassification_kable(
				brute_data$weighted_col_coclassification_matrix[species_output_order,species_output_order],
	...
	\begin{subfigure}{\textwidth}
		...
		<<echo=FALSE>>=
			weighted_coclassification_kable(
				brute_data$weighted_row_coclassification_matrix[species_output_order,species_output_order],
				colourmap="coclassification", diagonal_colour="coclassification diagonal"
			)
		@
