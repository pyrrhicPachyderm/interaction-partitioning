...
\begin{figure}
	...
	\caption{%
		...
		The red line shows the AIC value (\aicnum{\Sexpr{brute_data$fully_grouped_statistics$aic}}) of the full grouped model, where every pair os species shares a single interaction coefficient.
	}
	\label{fig:aic}
\end{figure}
