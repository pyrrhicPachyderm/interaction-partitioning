...
<<echo=FALSE>>=
	...
	partial_residuals_plot <- function(parameters, species_output_order) {
		plot_df <- c()
...
\begin{figure}
	<<echo=FALSE>>=
		plot(partial_residuals_plot(best_parameters, species_output_order))
	@
...
\begin{figure}
	<<echo=FALSE>>=
		plot(partial_residuals_plot(average_parameters, species_output_order))
	@
